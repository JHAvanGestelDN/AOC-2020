...
namespace Day2
{
    class Program
    {
        ...
        {
            string[] inputSample = new string[]
            {
                " 1 - 3 a: abcde  ",
                " 1-3 b: cdefg    ",
                " 2-9 c: ccccccccc",
            };
            string[] inputOne = new string[]
            {
                "                4-5 l: rllllj                             ",
            ...
            /*string[] fileReaderTest =
                System.IO.File.ReadAllLines(@"C:\Users\admin\RiderProjects\AdventOfCode\Assets\2.s.txt");
            Console.WriteLine(fileReaderTest);*/

            //Console.WriteLine(exerciseOne(CreateMap(inputOne)));
            //Console.WriteLine(ExerciseTwo(CreateMap(inputOne)));
        }

        public static int ExerciseOne(Dictionary<PswdRule, string> map)
        {
            int numberOfValidPasswords = 0;
            foreach (var keyValuePair in map)
            {
                int numberOfOccurrences = keyValuePair.Value.Count(x => x == keyValuePair.Key.Letter);
                if (numberOfOccurrences >= keyValuePair.Key.Min && numberOfOccurrences <= keyValuePair.Key.Max)
        ...
        public static int ExerciseTwo(Dictionary<PswdRule, string> map)
        {
            int numberOfValidPasswords = 0;
            foreach (var keyValuePair in map)
            {
                bool isValid = false;
                if (keyValuePair.Value[keyValuePair.Key.Min - 1] == keyValuePair.Key.Letter)
                    isValid = !isValid;
                if (keyValuePair.Value[keyValuePair.Key.Max - 1] == keyValuePair.Key.Letter)
                    isValid = !isValid;
                if (isValid)
                    numberOfValidPasswords++;
            }
        ...
        public static Dictionary<PswdRule, string> CreateMap(string[] input)
        {
            var map = new Dictionary<PswdRule, string>();
            foreach (string s in input)
            {
                string[] split = s.Trim().Split(':');
                map.Add(new PswdRule(split[0]), split[1].Trim());
            }
    ...
    internal class PswdRule
    {
        public char Letter { get; }
        public int Min { get; }
        public int Max { get; }

        public PswdRule(string rule)
        {
            string[] split = rule.Split('-', ' ');
            split = split.Where(x => !string.IsNullOrEmpty(x)).ToArray();
            this.Min = Convert.ToInt32(split[0]);
            this.Max = Convert.ToInt32(split[1]);
            this.Letter = Convert.ToChar(split[2]);
        }
    }
}